...
(function ($, window, document, undefined) {
    ...
    Embeds.prototype.parseUrl = function (url, pasted) {
        ...
        if (!(new RegExp(['youtube', 'youtu.be', 'vimeo', 'instagram', 'twitter','trislabs', 'facebook' , 'practisearena'].join('|')).test(url))) {
            $.proxy(this, 'convertBadEmbed', url)();
            return false;
        }

        html = url.replace(/\n?/g, '')
            .replace(/^((http(s)?:\/\/)?(www\.)?(youtube\.com|youtu\.be)\/(watch\?v=|v\/)?)([a-zA-Z0-9\-_]+)(.*)?$/, '<div class="video video-youtube"><iframe width="100%" height="400px" src="//www.youtube.com/embed/$7" frameborder="0" allowfullscreen></iframe></div>')
            .replace(/^https?:\/\/vimeo\.com(\/.+)?\/([0-9]+)$/, '<div class="video video-vimeo"><iframe src="//player.vimeo.com/video/$2" width="100%" height="281" frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe></div>')
            ...
            .replace(/^https?:\/\/www.trislabs\.com(\/.+)?\/([0-9]+)$/, '<div class="video video-vimeo"><iframe src="//www.trislabs.com/ide/$2" width="100%" height=700 frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe></div>')
            .replace(/^https?:\/\/www.practisearena\.com(\/.+)?\/([0-9]+)$/, '<div class="video video-vimeo"><iframe src="//www.practisearena.com/project/$2" width="100%" height=700 frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe></div>');
...
/*global MediumEditor*/
 ...
 (function ($, window, document, Util, undefined) {
    ...
    (function ($, window, document, undefined) {
        ...
        /** Default values */
        var pluginName = 'mediumInsert',
            addonName = 'Labs', // first char is uppercase
            defaults = {
                label: '<span class="fa fa-code-fork"></span>',
                placeholder: 'Enter virtual lab project link from Practicearena.com or TrisLabs.com ',
                captions: true,
                captionPlaceholder: 'Type caption (optional)',
                storeMeta: false,
                styles: {
        ...
        Labs.prototype.init = function () {
            var $labs = this.$el.find('.medium-insert-embeds');

            $labs.attr('contenteditable', false);
            $labs.each(function () {
                if ($(this).find('.medium-insert-embeds-overlay').length === 0) {
                    $(this).append($('<div />').addClass('medium-insert-embeds-overlay'));
                }
        ...
        Labs.prototype.events = function () {
            $(document)
                .on('click', $.proxy(this, 'unselectEmbed'))
                .on('keydown', $.proxy(this, 'removeEmbed'))
                .on('click', '.medium-insert-embeds-toolbar .medium-editor-action', $.proxy(this, 'toolbarAction'))
        ...
        Labs.prototype.backwardsCompatibility = function () {
            var that = this;
        ...
        Labs.prototype.editorSerialize = function () {
            var data = this._serializePreEmbeds();

            $.each(data, function (key) {
                var $data = $('<div />').html(data[key].value),
                    $labs = $data.find('.medium-insert-embeds');

                $labs.removeAttr('contenteditable');
                $labs.find('figcaption').removeAttr('contenteditable');
                $data.find('.medium-insert-embeds-overlay').remove();
        ...
        Labs.prototype.togglePlaceholder = function (e) {
            var $place = $(e.target),
                selection = window.getSelection(),
                range, $current, text;
        ...
        Labs.prototype.fixRightClickOnPlaceholder = function (e) {
            this.core.moveCaret($(e.target));
        };
        ...
        Labs.prototype.processLink = function (e) {
            var $place = this.$el.find('.medium-insert-embeds-active'),
                url;
        ...
        Labs.prototype.processPasted = function (e) {
            var pastedUrl, linkRegEx;
            if ($(".medium-insert-embeds-active").length) {
                return;
            }
        ...
        Labs.prototype.oembed = function (url, pasted) {
            var that = this;
            console.log(url);
        ...
        Labs.prototype.parseUrl = function (url, pasted) {
            var html;
        ...
        Labs.prototype.embed = function (html, pastedUrl) {
            var $place = this.$el.find('.medium-insert-embeds-active'),
                $div;
         ...
         * @return {void}
         */
        Labs.prototype.convertBadEmbed = function (content) {
            var $place, $empty, $content,
                emptyTemplate = this.templates['src/js/templates/core-empty-line.hbs']().trim();
        ...
        Labs.prototype.selectEmbed = function (e) {
            var that = this,
                $embed;
            if (this.core.options.enabled) {
                $embed = $(e.target).hasClass('medium-insert-embeds') ? $(e.target) : $(e.target).closest('.medium-insert-embeds');
        ...
        Labs.prototype.unselectEmbed = function (e) {
            var $el = $(e.target).hasClass('medium-insert-embeds') ? $(e.target) : $(e.target).closest('.medium-insert-embeds'),
                $embed = this.$el.find('.medium-insert-embeds-selected');
        ...
        Labs.prototype.removeEmbed = function (e) {
            var $embed, $empty;
        ...
        Labs.prototype.addToolbar = function () {
            var $embed = this.$el.find('.medium-insert-embeds-selected'),
                active = false,
                $toolbar, $toolbar2, mediumEditor, toolbarContainer;
        ...
        Labs.prototype.autoRepositionToolbars = function () {
            setTimeout(function () {
                this.repositionToolbars();
                this.repositionToolbars();
            }.bind(this), 0);
        ...
        Labs.prototype.repositionToolbars = function () {
            var $toolbar = $('.medium-insert-embeds-toolbar'),
                $toolbar2 = $('.medium-insert-embeds-toolbar2'),
                $embed = this.$el.find('.medium-insert-embeds-selected'),
                elementsContainer = this.core.getEditor().options.elementsContainer,
        ...
        Labs.prototype.toolbarAction = function (e) {
            var $button = $(e.target).is('button') ? $(e.target) : $(e.target).closest('button'),
                $li = $button.closest('li'),
                $ul = $li.closest('ul'),
                $lis = $ul.find('li'),
        ...
        Labs.prototype.toolbar2Action = function (e) {
            var $button = $(e.target).is('button') ? $(e.target) : $(e.target).closest('button'),
                callback = this.options.actions[$button.data('action')].clicked;
        ...
        $.fn[pluginName + addonName] = function (options) {
            return this.each(function () {
                if (!$.data(this, 'plugin_' + pluginName + addonName)) {
                    $.data(this, 'plugin_' + pluginName + addonName, new Labs(this, options));
                }
            });
        };
